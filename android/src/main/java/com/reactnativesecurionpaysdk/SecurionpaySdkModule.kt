...
class SecurionpaySdkModule(reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext) {
  ...
  fun LaunchPayment(cardnumber : String, expirymonth : String, expiryyear : String, cvv : String, amount: Int , currency :String , promise: Promise){
    ...
    if(!this.initialised){
      return promise.reject("NOT_INITIALISED", "Please run Init(publickey , signature) first")
    }
    ...
    securionPay.createToken(token){
      ...
      when (token.status) {
        Status.SUCCESS -> {
          securionPay.authenticate(
            ...
            when (authenticatedToken.status) {
              ...
              Status.ERROR -> {  val resultData: WritableMap = WritableNativeMap()
                resultData.putString("error",authenticatedToken.error.toString())
                promise.reject("AUTH_FAILED",resultData)
              }
            }
          }
        ...
        Status.ERROR -> {
          ...
          promise.reject("TOKEN_RETRIEVE_FAILED",resultData)
        }
